...
void BiCGSTAB<T>::solve(){
    ...
    while (k < n){
        ...
        sk = rk - alpha*vk;
        double sk_norm_sq = norm_sq(sk);      
        std::cout << "step: " << float(k) + 0.5 << ", (sk,sk)^1/2 = " << std::sqrt(sk_norm_sq) << "\n\n";
        check = std::chrono::high_resolution_clock::now();
        logs << float(k) + 0.5 << ',' 
             << std::chrono::duration_cast<std::chrono::microseconds>(check-start).count() << "," 
             << std::sqrt(sk_norm_sq) << ',' 
             << matvec_count << '\n';
        if (sk_norm_sq < epsilon*epsilon){
            xk += alpha * pk;
            break;
        }
        tk = A*sk;
        ...
        rkp1 = sk - omega * tk;
        double rkp1_norm_sq = norm_sq(rkp1);
        std::cout << "step: " << k + 1 << ", (rk,rk)^1/2 = " << std::sqrt(rkp1_norm_sq) << "\n\n";// << "xk = \n" << xk << "\n\n\n\n";
        check = std::chrono::high_resolution_clock::now();
        logs << k + 1 << ',' 
             << std::chrono::duration_cast<std::chrono::microseconds>(check-start).count() << "," 
             << std::sqrt(rkp1_norm_sq) << ',' 
             << matvec_count <<'\n';
        if (rkp1_norm_sq < epsilon*epsilon)
            break;
